...
export default {
  ...
  methods: {
    getRecords (filter) {
      ...
      return this.$axios.get('movimientos/bd_movimientos.php/donativos', { params: objFilter })
        .then(response => {
  ...
  mounted () {
    if (this.value.registrosSeleccionados && Object.keys(this.value.registrosSeleccionados).length > 0) { // si ya hemos cargado previamente los recargo al volver a este tab
      ...
      this.filterRecord = { codEmpresa: this.user.codEmpresa, tipoOperacion: 'DONATIVO' }
