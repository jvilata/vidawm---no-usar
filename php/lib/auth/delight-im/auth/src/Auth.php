...
class Auth {
	 ...
	 */
	public function __construct($databaseConnection, $useHttps = false, $allowCookiesScriptAccess = true, $ipAddress = null) {
	 ...
	 */
	public function register($email, $password, $username = null, $codEmpresa='01',callable $callback = null) {
		...
		try {
			$this->db->insert(
				...
				[
					...
					'username' => $username,
					'verified' => $verified,
				    'codEmpresa' => $codEmpresa, //jv.un usuario una o varias empresas
					'registered' => time()
				]
			);
		}
	...
	public function login($email, $password, $rememberDuration = null) {
		...
		if (!empty($userData)) {
			if (password_verify($password, $userData['password'])) {
				...
				if ($userData['verified'] === 1 || $userData['verified'] === "1") {
